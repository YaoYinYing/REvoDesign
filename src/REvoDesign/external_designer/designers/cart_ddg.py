...
class ddg(ExternalDesignerAbstract):
    ...
    def initialize(self):
        ...
        self.ddg_runner= CartesianDDG(pdb=make_temperal_input_pdb(
            molecule=self.molecule, reload=self.reload
        ),save_dir='cart_ddg_results',job_id=os.path.basename(self.pdb_filename)[:-4], node=node_picker(self.node_hint, **self.node_config))
