...
def is_run_node_available() -> bool:
    ...
    if node_hint.startswith('wsl'):
        ...
        try:
            ...
            warnings.warn(issues.PlatformNotSupportedWarning(f'Invalid Node configuration: {node_hint}'))
            return False
...
def is_docker_available() -> bool:
    ...
    try:
        ...
        warnings.warn(issues.PlatformNotSupportedWarning(f'Docker is not available: {e}'))
...
class ddg(ExternalDesignerAbstract):
    ...
    def __init__(self, molecule: str, **kwargs):
        ...
        self.use_legacy: bool=bool(self.bus.get_value('rosetta.cart_ddg.use_legacy', default_value=False))
        self.ddg_iterations: int=int(self.bus.get_value('rosetta.cart_ddg.iterations',converter=int, default_value=3))
        self.node_config: Optional[Dict[str, str]] =dict(self.bus.get_value('rosetta.node_config'))
    ...
    def scorer(self, mutant: Union[Mutant, RosettaPyProteinSequence ], **kwargs) -> float:
        if isinstance(mutant, Mutant):
            mutfile=mutants2mutfile([mutant], file_path=os.path.join('cart_ddg_results', 'mutfiles',f'{mutant.raw_mutant_id}.mutfile'))
        else:
        ...
        ddg_mut_id="MUT_"+ "_".join(f'{m.position}{IUPACData.protein_letters_1to3[m.mut_res]}' for m in mutant.mutations)
        score=ddg_value_df[ddg_value_df['Baseline'] == ddg_mut_id]['ddG_cart'].values[0]
        return float(score)

    __bibtex__ = {'Cartesian-ddG': """@article{doi:10.1021/acs.jctc.6b00819,
