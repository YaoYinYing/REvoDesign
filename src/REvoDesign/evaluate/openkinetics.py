"""Helpers for collecting and preparing OpenKinetics fixture data.

This module intentionally avoids importing REvoDesign's GUI/PyMOL-heavy
modules so it can be used from developer scripts and mocked tests.
"""
...
class ProteinChainSequence:
    """A protein sequence together with its original PDB residue numbering."""
    ...
    @classmethod
    def from_pdb(cls, pdb_path: str | Path, chain_id: str = "A") -> "ProteinChainSequence":
        ...
        for residue in model[chain_id]:
            ...
            residues.append((residue_number, residue_code))
        ...
        return cls(
            chain_id=chain_id,
            sequence="".join(code for _, code in residues),
            residue_numbers=tuple(number for number, _ in residues),
        )

    def residue_index(self, pdb_position: int) -> int:
        try:
            return self.residue_numbers.index(int(pdb_position))
        except ValueError as exc:
            raise OpenKineticsValidationError(
                f"Residue position {pdb_position} is not present in chain {self.chain_id}"
            ) from exc

    def residue_at(self, pdb_position: int) -> str:
        return self.sequence[self.residue_index(pdb_position)]

    def apply_mutation(self, mutation: "PointMutation") -> str:
        if mutation.chain_id != self.chain_id:
            raise OpenKineticsValidationError(
                f"Mutation {mutation.label} targets chain {mutation.chain_id}, expected {self.chain_id}"
            )

        residue_index = self.residue_index(mutation.position)
        wt_residue = self.sequence[residue_index]
        if wt_residue != mutation.wt_res:
            raise OpenKineticsValidationError(
                f"WT residue mismatch for {mutation.label}: PDB sequence has {wt_residue} at {mutation.position}"
            )

        mutant_sequence = list(self.sequence)
...
def build_variant_rows(
    chain_sequence: ProteinChainSequence,
    ...
    rows = [
        {
            "variant_id": "WT",
            "mutation": "WT",
            "protein_sequence": chain_sequence.sequence,
        }
    ]
    for label in mutation_labels:
        mutation = parse_point_mutation_label(label, default_chain_id=chain_sequence.chain_id)
        rows.append(
            {
                "variant_id": label,
                "mutation": label,
                "protein_sequence": chain_sequence.apply_mutation(mutation),
            }
        )
    return rows
    ...
    def _require_api_key(self) -> str:
        ...
        if not api_key:
            raise OpenKineticsConfigurationError(
                f"Missing OpenKinetics API key. Set {self.api_key_env} in the environment."
            )
        return api_key
...
class OpenKineticsScorer:
    ...
    def __init__(
        ...
        *,
        ...
        base_url: str = DEFAULT_OPENKINETICS_BASE_URL,
        api_key_env: str = DEFAULT_OPENKINETICS_API_KEY_ENV,
        default_method: str = DEFAULT_OPENKINETICS_METHOD,
        default_prediction_type: str = DEFAULT_OPENKINETICS_PREDICTION_TYPE,
        poll_interval_seconds: int = DEFAULT_OPENKINETICS_POLL_INTERVAL_SECONDS,
        timeout_seconds: int = DEFAULT_OPENKINETICS_TIMEOUT_SECONDS,
        max_retries: int = 3,
        ...
        self.default_prediction_type = default_prediction_type
        self.poll_interval_seconds = poll_interval_seconds
        self.timeout_seconds = timeout_seconds
        self.cache_enabled = cache_enabled
        self.cache_dir = os.path.expanduser(cache_dir or _default_openkinetics_cache_dir())
...
def collect_openkinetics_fixture_dataset(
    *,
    ...
    chain_sequence = ProteinChainSequence.from_pdb(pdb_path, chain_id=chain_id)
    ...
    variant_rows = build_variant_rows(chain_sequence, mutation_labels)
    ...
    manifest = {
        ...
        "api_key_policy": f"Read from environment variable {api_key_env} and never stored",
