...
class Evalutator:
    ...
    def activate_focused(self):
        ...
        logging.debug(f"Current Mutant ID: {self.mutant_tree_candidates.current_mutant_id}")
        ...
        if self.mutant_tree_candidates.current_mutant_id:
            ...
            cmd.hide("cartoon", f"{self.mutant_tree_candidates.current_mutant_id}")
        ...
        all_enabled_mutants_in_current_group = [
            mutant
            for mutant in cmd.get_object_list(f"({self.mutant_tree_candidates.current_branch_id})")
            if mutant != self.mutant_tree_candidates.current_mutant_id and mutant in all_enabled_mutant_ids
        ]
        ...
        if self.mutant_tree_candidates.current_branch_id:
            ...
            cmd.group(self.mutant_tree_candidates.current_branch_id, action="open")
    ...
    def mutant_decision(self, decision_to_accept: bool):
        ...
        if not self.is_this_pymol_object_a_mutant(self.mutant_tree_candidates.current_mutant_id):
            logging.warning(f"Ingoring non mutant {self.mutant_tree_candidates.current_mutant_id}")
            return
        ...
        if decision_to_accept:
            self.mutant_tree_pssm_selected.add_mutant_to_branch(
                ...
                mutant_obj=self.mutant_tree_candidates.mutant_tree[self.mutant_tree_candidates.current_branch_id][
            ...
            if (
                ...
                logging.warning(f"{self.mutant_tree_candidates.current_branch_id} does not exist. skipped")
                return
    ...
    def walk_mutant_groups(
        ...
        set_widget_value(
            progressBar_mutant_choosing,
            self.mutant_tree_candidates.get_mutant_index_in_all_mutants(current_mutant_id),
        )
        ...
        logging.info(f'Walked to the {"next" if walk_to_next else "previous"} mutant {current_mutant_id}.')
    ...
    def jump_to_branch(self):
        ...
        logging.info(f"Progressbar set to {progress}: {self.mutant_tree_candidates.current_mutant_id}")
        set_widget_value(progressBar_mutant_choosing, progress)

        # Setting mutant ids to candidates box
        set_widget_value(
            comboBox_mutant_ids,
            list(self.mutant_tree_candidates.get_a_branch(branch_id=branch).keys()),
        )
        set_widget_value(
            comboBox_mutant_ids,
            self.mutant_tree_candidates.current_mutant_id,
    ...
    def jump_to_a_mutant(self):
        ...
        if mutant_id not in self.mutant_tree_candidates.get_a_branch(branch_id=branch_id):
            logging.error(
    ...
    def find_all_best_mutants(self):
        ...
        comboBox_mutant_ids = self.bus.ui.comboBox_mutant_ids
        if self.mutant_tree_candidates.empty:
            logging.error("No available mutant tree. Please reinitialize it before picking mutants.")
            return
        ...
        for branch_id in self.mutant_tree_candidates.all_mutant_branch_ids:
            ...
            logging.info(f"Best hit of {self.mutant_tree_candidates.current_mutant_id} accepted.")
    ...
    def is_this_pymol_object_a_mutant(self, mutant):
        _mutant_obj = extract_mutant_from_pymol_object(pymol_object=mutant, sequences=self.designable_sequences)
        return _mutant_obj is not None

    def recover_mutant_choices_from_checkpoint(self, mutant_choice_checkpoint_fn):
        lcdNumber_selected_mutant = self.bus.ui.lcdNumber_selected_mutant
        ...
        self.mutant_tree_pssm_selected = self.mutant_tree_candidates.create_mutant_tree_from_list(
    ...
    def initialize_design_candidates(
        ...
        lineEdit_output_mut_txt = self.bus.get_widget_from_cfg_item("ui.evaluate.input.to_mutant_txt")
        ...
        lineEdit_output_mut_txt.setEnabled(not self.mutant_tree_candidates.empty)
        ...
        # clean the view
        cmd.disable(" or ".join(self.mutant_tree_candidates.all_mutant_branch_ids))
        cmd.hide("sticks", " or ".join(self.mutant_tree_candidates.all_mutant_ids))
        ...
        pushButton_accept_this_mutant.clicked.connect(partial(self.mutant_decision, True))
        pushButton_reject_this_mutant.clicked.connect(partial(self.mutant_decision, False))
