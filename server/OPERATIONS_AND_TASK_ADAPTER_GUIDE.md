...
Pick the family whose stack matches the new tool before creating a new one.
Versions are what the pinned `docker/runners/<family>/Dockerfile` installs —
re-read the file when matching, do not trust this table after a bump.

| Family | Tasks | Base image / CUDA | Python | Frameworks | GPU |
| --- | --- | --- | --- | --- | --- |
| `gremlin` | gremlin | condaforge/mambaforge (legacy conda env) | 2/3 legacy | GREMLIN conda env | no |
