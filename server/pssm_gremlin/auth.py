...
# ---------------------------------------------------------------------------
# Migrate users from legacy users.txt into the SQLite user database
# ---------------------------------------------------------------------------


def _parse_users_file(path: str) -> list[dict[str, str]]:
    """Parse a legacy users.txt file and return a list of {username, password} dicts."""
    ...
    with open(path) as fh:
        for line in fh:
            ...
            try:
                ...
                logging.warning("Skipping malformed line in users file: %s", stripped)
                continue
            entries.append({"username": username.strip(), "password": password.strip()})
    return entries


def migrate_users_file(user_db: UserDatabase, users_file_path: str, admin_usernames: set[str]) -> int:
    """Import users from a legacy ``users.txt`` file into the SQLite database.

    Existing users (by username) are skipped.  Passwords are re-hashed with
    the Werkzeug scheme.  Returns the number of newly imported users.
    """
    entries = _parse_users_file(users_file_path)
    imported = 0
    for entry in entries:
        ...
        user_db.create_user(
            username=username,
            email=f"{username}@revodesign.local",
            password=entry["password"],
            is_admin=username in admin_usernames,
        )
        # ponytail: auto-verify imported users so existing credentials work immediately
        user = user_db.get_user_by_username(username)
        if user:
            user_db.verify_email(user["id"])
        imported += 1
        logging.info("Migrated legacy user %r into SQLite user database", username)
    return imported
