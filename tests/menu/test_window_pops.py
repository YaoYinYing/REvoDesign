...
from REvoDesign.shortcuts.wrappers.rosetta_tasks import (wrapped_fast_relax, wrapped_pross, wrapped_relax_w_ca_constraints, wrapped_rosettaligand)
from REvoDesign.shortcuts.wrappers.represents import (wrapped_color_by_mutation, wrapped_color_by_plddt, wrapped_real_sc)
from REvoDesign.shortcuts.wrappers.mutation_effect_predictors import wrapped_thermompnn
from REvoDesign.shortcuts.wrappers.exports import wrapped_dump_fasta_from_struct, wrapped_menu_dump_sidechains
from REvoDesign.shortcuts.wrappers.ligand_converters import wrapped_smiles_conformer_batch, wrapped_smiles_conformer_single, wrapper_sdf2rosetta_params
...
def test_menu_window_pops(test_window_wrapper,test_window_wrapper_func, test_worker: TestWorker):
    ...
    assert not hasattr(test_worker.plugin.bus.ui, 'open_windows'), f"There are open windows stored in the bus: {getattr(test_worker.plugin.bus.ui, 'open_windows')}"
    ...
    assert not hasattr(test_worker.plugin.bus.ui, 'open_windows'), f"There are remaining window opened: {getattr(test_worker.plugin.bus.ui, 'open_windows')}"
