...
from REvoDesign.shortcuts.wrappers.utils import (
    wrapped_convert_residue_ranges,
...
@pytest.mark.parametrize(
    "test_window_wrapper, test_window_wrapper_func",
    (
...
def test_menu_window_pops(test_window_wrapper, test_window_wrapper_func, test_worker: TestWorker):
    ...
    assert not hasattr(
        test_worker.plugin.bus.ui, "open_windows"
    ), f"There are open windows stored in the bus: {getattr(test_worker.plugin.bus.ui, 'open_windows')}"
    ...
    assert not hasattr(
        test_worker.plugin.bus.ui, "open_windows"
    ), f"There are remaining window opened: {getattr(test_worker.plugin.bus.ui, 'open_windows')}"
