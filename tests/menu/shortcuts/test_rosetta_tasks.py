...
def test_pross(test_worker: TestWorker, test_node_hint):
    ...
    refined_pdbs=[x for x in os.listdir(os.path.join(run_dir, 'refinement','pross_refinement', 'pdb')) if x.endswith('.pdb')]
    ...
    assert [x for x in os.listdir(os.path.join(filter_scan_dir, 'resfiles')) if x.startswith('designable_aa_resfile')], f'{filter_scan_dir}/resfiles should contain resfiles'
    ...
    assert [x for x in os.listdir(os.path.join(design_dir, 'scorefile')) if x.endswith('.sc')], f'{design_dir}/scorefile should contain score files ends with .sc'
