...
class TestRosettaTasks:
    ...
    def test_rosetta_ligand(self, job_id, start_from, cst, test_node_hint, mock_rosetta_node_config):
        ...
        assert [
            x for x in os.listdir(os.path.join(run_dir, "scorefile")) if x.endswith(".sc")
        ], f"{run_dir}/scorefile should contain score files ends with .sc"
    ...
    def test_fast_relax(self, job_id, pdb, dualspace, ligand, test_node_hint, mock_rosetta_node_config):
        ...
        assert [
            x for x in os.listdir(os.path.join(run_dir, "all")) if x.endswith(".pdb")
        ], f"{run_dir}/all should contain pdb files"
        assert [
            x for x in os.listdir(os.path.join(run_dir, "all")) if x.endswith(".sc")
        ], f"{run_dir}/all should contain score files ends with .sc"
