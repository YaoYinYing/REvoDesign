...
def test_shortcut_dump_fasta_from_struct(
    format, chain_id, drop_missing_residue, suffix, expected_seq_num, test_worker: TestWorker
):
    ...
    expected_fasta_file = os.path.join(
        "dumped_sequences", f'9gbw_{"".join(chain_id)}{f"_{suffix}" if suffix else ""}.{format}'
    )
