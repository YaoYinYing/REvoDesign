...
def test_shortcut_smiles_conformer_single(test_worker: TestWorker):
    ...
    assert os.path.isfile(
        os.path.join(os.path.abspath("./ligands_conformers/"), f"{ligand_name}.sdf")
    ), f'{ligand_name}.sdf not found in the directory {os.path.abspath("./ligands_conformers/")}'
    ...
    assert (
        os.path.getsize(conformer_fa_pdb) > 0
    ), f"{ligand_name}.fa_conformers.pdb should not be empty in the directory {res_dir}/{ligand_name}"
    conformer_pdb_contents = open(conformer_fa_pdb).readlines()
    assert (
        len([x for x in conformer_pdb_contents if x.startswith("TER")]) > 1
    ), f"{ligand_name}.fa_conformers.pdb should have multiple TER record in the directory {res_dir}/{ligand_name}"
...
def test_shortcut_smiles_conformer_batch(test_worker: TestWorker):
    ...
    shortcut_smiles_conformer_batch(smiles=smiles, num_conformer=20, save_dir=res_dir, n_jobs=1, show_conformer="None")
    ...
    for k in smi:
        ...
        assert os.path.isfile(
            os.path.join(params_dir, f"{k}.fa.params")
        ), f"{k}.fa.params not found in the directory {res_dir}"
        assert (
            os.path.getsize(os.path.join(params_dir, f"{k}.fa.params")) > 0
        ), f"{k}.fa.params should not be empty in the directory {res_dir}"
...
def test_shortcut_sdf2rosetta_params(test_worker: TestWorker):
    ...
    shortcut_sdf2rosetta_params(ligand_name="HEM", sdf_path=sdf_path, save_dir=save_dir)
    res_dir = os.path.join(save_dir, "HEM")
    assert (
        len(os.listdir(res_dir)) == 6
    ), f"Expected 6 files in the directory {res_dir}/HEM, but got: \n {os.listdir(res_dir)}"
    assert os.path.isfile(
        os.path.join(res_dir, "HEM.fa.params")
    ), f"HEM.fa.params not found in the directory {res_dir}/HEM"

    assert (
        os.path.getsize(os.path.join(res_dir, "HEM.fa.params")) > 0
    ), f"HEM.fa.params is empty in the directory {res_dir}/HEM"
    assert (
        os.path.getsize(os.path.join(res_dir, "HEM.fa_conformers.pdb")) == 0
    ), "HEM.fa_conformers.pdb should be empty since there is only one conformer."
