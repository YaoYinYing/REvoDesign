...
@pytest.mark.skipif(
        not RfDiffusion.installed, reason="RfDiffusion not installed"
    )
def test_rfdiffusion_general():
    ...
    assert sequence_designed.chains[0].sequence != sequence_original.chains[0].sequence, f'Expected different sequences: {sequence_designed.chains[0].sequence=} != {sequence_original.chains[0].sequence=}'

    assert sequence_designed.chains[0].sequence.startswith('GGGGGG'), f'Expected sequence to start with GGGGGG: {sequence_designed.chains[0].sequence=}'