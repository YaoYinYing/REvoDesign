...
class TestNonEnglishInput:
    ...
    def test_non_english_input(
        self, drop_space_with_underline, lan, non_eng_dirname, filename, test_worker: TestWorker, test_tmp_dir
    ):
        test_worker.test_id = test_worker.method_name()
        ...
        test_worker.save_screenshot(widget=test_worker.plugin.window, basename=base_name)
