...
class PssmAnalyzer():
    ...
    def plot_custom_indices_segments(self, df_ori, sequence, pop=False, annotate=False,
                                     custom_indices_str='', alias='undefined', design_case='default',
                                     cutoff=[-100, 100], preferred_substitutions=None):
        df = df_ori.copy()
        ...
        for _, resid in enumerate(custom_indices[1:]):
            ...
            mutation_candidates['mutations'][resid] = {"wt": wt_aa, "wt_pssm": pssm_scores.loc[wt_aa],
                                                    "candidates": {}}
    ...
    def parse_preffered_mutation_string(self,preffered_str):
        preffered_dict = {_preffered_sub[0]: [res for res in _preffered_sub[2:]]
    ...
    def design_protein_using_pssm(self, sequence, alias, preffered=[], design_case='default', custom_indices_fp='',
        ...
        if len(preffered) > 0:
            preffered_dict = self.parse_preffered_mutation_string(preffered_str=preffered)
    ...
    @staticmethod
    def process_position(position, wt_residue, wt_pssm_score, candidates, input_pse, molecule, chain_id, reject, create_full_pdb,
         max_abs_pssm, cmap):
        
        logging.info(f'Runing mutagenesis test for position {position}{wt_residue}...')
        ...
        for new_residue, new_residue_score in candidates.items():
            if reject and (new_residue in reject or wt_residue in reject):
                logging.info(f"Skipping rejected mutation: {position}{wt_residue} to {new_residue}")
                refresh_window()
                continue
            else:
                refresh_window()
    ...
    def load_mutants_to_pymol_session(self,input_pse, output_pse, molecule, chain_id, mutant_json, reject, create_full_pdb,
                                      progress_bar, nproc):
    ...
    def merging_sessions(self,):
        ...
        merge_sessions(session_paths=[self.input_pse, merged_temp_session],
