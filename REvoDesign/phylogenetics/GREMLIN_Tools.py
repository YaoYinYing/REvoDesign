...
class GREMLIN_Tools:
    ...
    # Rest of the functions can be defined similarly as methods within the class
    # ## Functions for prepping the MSA (Multiple sequence alignment)
    # from fasta
    def parse_fasta(self, filename, limit=-1):
    ...
    def filt_gaps(self, msa, gap_cutoff=0.5):
    ...
    def plot_mtx(self, key="zscore", vmin=1, vmax=3):
        ...
        plt.imshow(squareform(self.mtx[key]), cmap='Blues', interpolation='none', vmin=vmin, vmax=vmax)
        plt.grid(False)
        #plt.show()
    ...
    def plot_w(self, i, j, i_aa, j_aa, idx=0):
        ...
        plt.text(wt_j_index, wt_i_index, 'WT', color='k', ha='center', va='center', fontsize=9)

        plt.title(f"Top.{idx:02}: W for positions {i_aa} and {j_aa}")
        plot_fp = f"{self.pwd}/Top.{idx:02}.W_for_positions_{i_aa}_and_{j_aa}.png"
        plt.savefig(plot_fp)
    ...
    def analyze_coevolving_pairs_for_i(self, i):
        ...
        matching_indices = np.where((self.mrf["w_idx"][:, 0] == i) | (self.mrf["w_idx"][:, 1] == i))[0]
        ...
        for idx in matching_indices:
            ...
            if self.pd_mtx['j'][idx] - self.pd_mtx['i'][idx] > 5:
                coevolving_pairs.append((i, j, self.pd_mtx['i_aa'][idx], self.pd_mtx['j_aa'][idx], self.pd_mtx['zscore'][idx]))
