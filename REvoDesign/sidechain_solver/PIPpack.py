...
class PIPPack_worker:
    ...
    def __init__(self, pdb_file:str, use_model:str='ensemble'):
        ...
        cfg=reload_config_file('sidechain-solver/pippack')['sidechain-solver']
    ...
    def run_mutate(
        ...
        **kwargs,
        ...
        mutant_sequence=[list([v.replace('X','') for v in mutant_obj.get_mutant_sequences().values()])]

        temp_dir = tempfile.mkdtemp(prefix='RD_design_pipp')
        temp_pdb_path = os.path.join(temp_dir, f"{new_obj_name}.pdb")

        self.pippack_worker._run_repack_single(pdb_file=self.pdb_file,output_file=temp_pdb_path,mutant_sequence=mutant_sequence)
