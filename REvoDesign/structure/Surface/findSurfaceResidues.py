...
def determine_surface_residue(input_file, output_file, molecule, chain_id='A', exclude_residue_selection='',cutoff=15, do_show_surf_CA=True):
    # cmd.reinitialize()
    # cmd.load(input_file)
    cmd.save(input_file)
    ...
    surface_selection = findSurfaceResidues(selection=molecule, cutoff=cutoff, return_selection=1)

    logging.debug(f'{molecule} and (not {surface_selection} ) and n. CA {_sel_exclude_residue_selection}')

    cmd.create(f'ner_ca_{cutoff}', f'{molecule} and (not {surface_selection} ) and n. CA {_sel_exclude_residue_selection}')
    cmd.create(f'er_ca_{cutoff}', f'{molecule} and {surface_selection} and n. CA {_sel_exclude_residue_selection}')
    ...
    surface_residue_ids = list(set([int(atom.resi) for atom in cmd.get_model(f'er_ca_{cutoff}').atom]))
    surface_residue_ids.sort()
    residue_filename = os.path.join('surface_residue_records', f'residues_cutoff_{cutoff}.txt')
