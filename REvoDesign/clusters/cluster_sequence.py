...
class Clustering:
    ...
    def global_alignment(self, seqs,indexes):
        ...
        return (''.join(str(r.sequences[0]).split('-')), ''.join(str(r.sequences[1]).split('-')), r.score, r[0].aligned[0][0][0], r[0].aligned[0][0][1], i, j)
    ...
    def set_and_write_clusters(self,progressbar):
        ...
        def processing(paramlist, indexlist, batch_size, mode='w'):
            ...
            with open(self.buffer_file, mode, ) as bw:
                ...
                for sub_indexlist,sub_paramlist in zip(minibatches_generator(indexlist, batch_size),minibatches_generator(paramlist, batch_size)):
                    batch_count += 1
                    start_time = time.perf_counter()
                    ...
                    # parallel executor 
                    parallel_executor = ParallelExecutor(self.global_alignment, args_list, self.num_proc-1)
        ...
        self.plot_score_mtx(df_score, vmin=min(df['Score'].to_list()), vmax=max(df['Score'].tolist()))
