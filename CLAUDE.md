...
- **PR lessons learned**: Do not repeatedly trigger automated reviews; request one review pass, batch valid findings, and verify locally between pushes. Runner builds should use the final Docker tag directly—do not create `:next` or `:previous` image tags. Replacing `:latest` retires the old digest through dangling-image pruning. For SIFs, record the exact source Docker digest beside the staged file and refuse activation on mismatch.
