...
SIFs or validate Docker execution. Production SIFs are built manually to new
`.partial`/versioned paths while the healthy stack remains up, validated, then
atomically promoted before prepared activation.
...
Redis remains internal and currently has no authentication; setting an unused
password environment variable alone does not secure it.
