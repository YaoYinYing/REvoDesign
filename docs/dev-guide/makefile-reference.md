...
- **Temporary test directory**: Root `Makefile` test targets run inside `tmp-test-dir-with-unique-name/` to ensure the *installed* package is tested, not the source tree. The conftest resolves `REPO_DIR` via `os.path.dirname(__file__)` and `TEST_ROOT` via `os.path.abspath(".")` relative to CWD, which fails outside this directory.
